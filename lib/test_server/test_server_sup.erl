...
timetrap(Timeout0, Pid) ->
    ...
    Timeout1 = case test_server:purify_is_running() of
		  true when integer(Timeout0) ->
		      5 * Timeout0;
		  false ->
		      Timeout0
	      end,
    Timeout2 = case get_os_family() of
		  vxworks ->
		      5 * Timeout1;
		  _ ->
		      Timeout1
    ...
    after trunc(Timeout) ->
	    ...
	    exit(Pid,{timetrap_timeout, trunc(Timeout), Line})
    end.
...
format_loc({Mod,Line}) when is_integer(Line) -> 
	...
	[$E,$T,$I,$U,$S,$_|_]  ->
	    io_lib:format("{~w,<a href=\"~s~s#~w\">~w</a>}",
			  [Mod,downcase(ModStr),?src_listing_ext,
			   round_to_10(Line),Line]);
	_ ->
	    io_lib:format("~w",[{Mod,Line}])
    end;
format_loc(Loc) ->
    io_lib:format("~p",[Loc]).    
