...
** TODO Does uTP allow for half-open connections?
...
*** TODO 2: CONNECTED + pkt(st_fin)
    Mark where the end of the stream is
    We should transition to got_fin in state
**** DONE What does libutp do?
     ...
     buffer closes.
**** TODO When the buffer closes, we should stop satisfying data to the upper layer
...
**** TODO When receiving a FIN packet, install a knowledge of this in the pkt_buffer
     We should record that we got a fin packet, and what seq_no is
     stamped with the fin.
**** TODO Set a Message which says we got the fin packet.
**** TODO Make sure we don't send data to the upper layer when a fin has been sent!
**** TODO When we confirm the eof_pkt from the reorder buffer, we should post a message this is the case
...
**** TODO If we know the eof_pkt and receive packets past it, throw them out!
...
*** TODO CONNECTED -> GOT_FIN
*** TODO FIN_SENT -> GOT_FIN
*** TODO GOT_FIN -> RESET
*** TODO RESET -> DESTROY
*** TODO RESET -> GOT_FIN (why does this happen?)
*** TODO GOT_FIN -> DESTROY_DELAY
*** TODO FIN_SENT -> DESTROY (timeout)
*** TODO DESTROY_DELAY -> DESTROY (timeout)
