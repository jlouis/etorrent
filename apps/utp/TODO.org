...
** TODO Implement the FIN_SENT state properly
*** TODO Refactor out pkt receives from the other end from connected
    We need this code present in the fin_sent state as well, so factor
    it out such that we can use it here as well.
*** TODO Implement the altered state on how to handle the next state in this.
** TODO How do we leave FIN_SENT?
