...
* TODO Fix the problem with delayed ACK messages on piggyback
  ...
  The fix is to try to fill the window straight away and if we sent
  data, we can forget about the ACK timer altogether because we just
  succeeded in getting data out on the socket.

  It suggests that the checking for the delayed ACK timer should
  happen later on in the order of code so we don't send out an ACK
  almost as soon as we detect something is odd here.
* TODO Make code which allows us to graph various counters on a running system
