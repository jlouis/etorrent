...
**** TODO What does libutp do?
     What happens when the incoming packet is of type st_fin?
     What do we record when this is the case?
     Do we alter the way we process packets afterwards?
       if so, how?
     What happens when we confirm the last packet and the reorder
       buffer is empty?
