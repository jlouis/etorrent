...
* TODO Fix the problem of sockets closing down
  ...
  wrong thing here.
* TODO Connect-N-Communicate can fail, figure out why
  Sometimes, the Connect-N-Communicate test fails with a timeout. We
  need to figure out why that happens and fix it. We get a timeout, so
  it is either

  ; Some case that is not handled
  ; A deadlock between the two endpoints

  This may be due to the timeout problem we saw before.
...
* Let the SYN-packet use the normal retransmission queue
  ...
  other end.
** Also, here we want to handle the case of bad packet transmission
