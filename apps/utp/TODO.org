...
* DONE When we disconnect due to an error, we better clean up
  There is something murky where we don't get a process moved to its
  destroy state. Thus, the process hangs around and is still there,
  even though I would have presumed it should have gone to death.

  To find this, run a repeated test of test_piggyback and then look up
  and query one of the processes that have troubles and figure out
  what its internal state is and what it is currently doing.

  We could attach a debugger to it, or send it a message for
  formatting its internal state.

  There is a call, sys:get_status/1,2 for this!

  State is destroy, so there is something that does not get cleaned up
...
* Packet size control
  ...
  about it.
* DONE Storing CID+1 should in principle be checked against overflow
