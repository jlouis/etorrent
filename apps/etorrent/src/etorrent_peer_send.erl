...
local_choke(S) ->
    etorrent_peer_states:local_choke(S#state.torrent_id,
                                  S#state.control_pid).
...
handle_info(rate_update, S) ->
    ...
    ok = etorrent_peer_states:send_rate(S#state.torrent_id,
                                     S#state.control_pid,
                                     Rate#peer_rate.rate),
...
handle_cast(unchoke,
    ...
    ok = etorrent_peer_states:local_unchoke(Torrent_Id, ControlPid),
